...
pub(crate) fn object_handle<T>(id: ffi::cl_uint) -> *mut T {
    id as usize as *mut T
}

pub(crate) fn object_id<T>(handle: *mut T) -> ffi::cl_uint {
    handle as usize as ffi::cl_uint
}
...
pub(crate) unsafe fn info_write(out: *mut ffi::cl_int, value: ffi::cl_int) {
    if !out.is_null() {
        unsafe { *out = value };
    }
}
